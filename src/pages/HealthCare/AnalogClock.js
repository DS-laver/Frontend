...
export default function AnalogClock() {
  ...
  const secondsPosition = getSecondPosition(secondsLength, secondsDegrees, center);

  return (
    <View style={[styles.clockContainter, { width: clockWidth, height: clockWidth, borderRadius: clockWidth / 2 }]}>
      <View style={[styles.clockFace, { borderRadius: clockWidth / 2 }]}></View>
      <View style={[styles.clockHourHandle, { width: 6, height: hourLength, left: hourPosition.x, top: hourPosition.y, transform: [{ rotate: `${hourDegrees}deg` }] }]}></View>
      <View style={[styles.clockMinitesHandle, { width: 4, height: minsLength, left: minsPosition.x, top: minsPosition.y, transform: [{ rotate: `${minsDegrees}deg` }] }]}></View>
      <View style={[styles.clockSecondHandle, { width: 2, height: secondsLength, left: secondsPosition.x, top: secondsPosition.y, transform: [{ rotate: `${secondsDegrees}deg` }] }]}></View>
    </View>
  );
}
